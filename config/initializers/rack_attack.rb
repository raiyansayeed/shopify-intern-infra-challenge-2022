...
#   end
  ...
  throttle("limit warehouse form submissions", limit: 5, period: 20.seconds) do |req|
    if req.path == "/warehouses" && req.post?
      req.ip
    end
  end

  throttle("limit inventory form submissions", limit: 5, period: 20.seconds) do |req|
    if req.path == "/inventories" && req.post?
        req.ip
    end
  end
end
